...
rm(list=ls())
df <- read.csv("results/ils/res_ils_moves.csv", sep=",", header = FALSE)
df <- data.frame(df)
...
# study on the number of moves per perturbation phase
moves <- merged.df %>%
  group_by(Moves) %>%
...
write.table(moves, "src/Stats/outputs/ils/moves_study.csv", sep = ",", row.names = FALSE, col.names = TRUE)